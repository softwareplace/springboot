...
val moduleName = "${sourceGroup}.graalvm"
    ...
    jar {
        ...
        manifest {
            attributes(
                ...
                "-exportcontents" to "${sourceGroup}.graalvm",
...
gradlePlugin {
    ...
    plugins {
        create("graalvm") {
            id = "$sourceGroup.graalvm"
            version = tagVersion
            implementationClass = "$sourceGroup.graalvm.BuildSourcePlugin"
        }
    }
}
...
publishing {
    publications {
        create<MavenPublication>("graalvm") {
            artifactId = "graalvm"
            from(components["java"])
            Shared.publishConfig(this, sourceGroup, "graalvm")
        }
    }
}
